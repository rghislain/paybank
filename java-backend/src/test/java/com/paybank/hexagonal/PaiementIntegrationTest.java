...
import com.paybank.hexagonal.adaptateurs.StripeMokbankAdaptateur;
...
//@ActiveProfiles("local") //Indique à Spring de charger application-local.properties

@TestPropertySource(properties = {
	    //"stripe.api.url=http://localhost:1234/v1/charges" //Forcer une fausse adresse locale accélère le refus immédiat au lieu d'attendre un timeout de 2 minutes
		...
		//"spring.datasource.url=${TEST_DB_URL:jdbc:postgresql://localhost:54322/postgres}"
		//"stripe.api.url=http://localhost:8080/api/v1/payments"
})
/*
@SpringBootTest(
...
class PaiementIntegrationTest {
    ...
    @Autowired
    private ExecutionPaiementUseCase executionPaiementUseCase;
    
    //private UUID clientId = UUID.randomUUID(); //.fromString("78459cde-ac48-411c-a42c-f396f2246d46");
    
    @Autowired
    private JdbcTemplate jdbcTemplate;
    
    @Autowired
    private AnnulerPaiementUseCase annulerPaiementUseCase; // Spring va injecter l'implémentation automatiquement
    
    @SpyBean //@MockBean
    private StripeMokbankAdaptateur stripeMokbankAdaptateur;
    
    @Autowired
    private ModifierPaiementUseCase modifierPaiementUseCase;
    ...
    void simulerUneAnnulationDePaiementReussie() {
    	...
    	//Mockito.when(stripeMokbankAdaptateur.annulerPaiement(Mockito.anyString())).thenReturn(true);
    	// On dit au Spy de ne pas exécuter la vraie méthode HTTP d'annulation, mais de simuler un succès
    	Mockito.doReturn(true) // Ou .doReturn(true) si la méthode renvoie un booléen
    	       .when(stripeMokbankAdaptateur)
    	       .annulerPaiement(Mockito.anyString()); // Mets le nom exact de ta méthode d'annulation
    	
        UUID transactionId = UUID.randomUUID(); //UUID.fromString("f0820811-0ddf-4a2a-871a-9232e441253a");  
        ...
        //insertion du client (respect des contraintes de celfs etrangeres)
        jdbcTemplate.update(
        ...
        // On simule une transaction existante réussie en BDD (Pré-requis)
        /*
        jdbcTemplate.update(
            "INSERT INTO paiements (id, client_id, montant_centimes, statut, cle_idempotence) VALUES (?::uuid, ?::uuid, ?, ?, ?)",
            transactionId, clientId, 10000, "SUCCESS", cleIdempotence
        );
        */
        // Déclenchement de l'annulation
        // (Être sûr d'avoir injecté le UseCase ou le Service dans le test)
        //AnnulationService annulationService=new AnnulationService(, null)
        ...
        annulerPaiementUseCase.executerAnnulationSurIDPaymentIntent(paymentIntentId); //(cleIdempotence);
    ...
    void simulerUneModificationDeMontantReussie() {
        // --- 1. PRÉPARATION : Création d'une transaction initiale ---
        UUID compteClientIdFictif = UUID.randomUUID();
        this.creerClient(compteClientIdFictif);
        ...
        MontantCentimes nouveauMontant = new MontantCentimes(nouveauMontantBrut); // Instanciation de ton Value Object

        System.out.println("--- DÉBUT DE LA MODIFICATION DU PAIEMENT ---");
        // Appel de ton Use Case fraîchement configuré
