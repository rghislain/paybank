...
import com.paybank.hexagonal.jpaRepository.SpringDataTransactionRepository;
import com.paybank.hexagonal.jpaRepository.TransactionRepository;

@Component
public class PersistanceTransactionRepositoryAdapteur implements TransactionRepository {
    private final SpringDataTransactionRepository springDataTransactionRepository;

    @Autowired
    private JdbcTemplate jdbcTemplate;
    
    public PersistanceTransactionRepositoryAdapteur(SpringDataTransactionRepository springDataRepo) {
    ...
    @Override
    public BigDecimal calculateMonthlyRevenueDate(YearMonth month) {
    	LocalDate start = month.atDay(1);
        LocalDate end = month.atEndOfMonth();
    	  	
        List<TransactionEntity> transactions = springDataTransactionRepository.findAllByDateRangeSorted(start, end);
