...
import com.paybank.hexagonal.jpaRepository.PasswordRepository;
...
public class AuthControleur {
    ...
    public ResponseEntity<?> modifierMdpStandard(@RequestBody MdpStandardRequest request) {
    	// Vérification de sécurité basée sur l'utilisateur authentifié (JWT / SecurityContext)
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if (authentication == null || !authentication.isAuthenticated()
                || "anonymousUser".equals(authentication.getPrincipal())) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Utilisateur non authentifié.");
        }
        // Récupération de l'utilisateur en base pour vérifier son rôle réel
        String email = authentication.getName();
        UtilisateurEntity userConnecte = utilisateurRepository.findByEmail(email).orElse(null);
        if (userConnecte == null || userConnecte.getRole() != Role.ADMIN) {
